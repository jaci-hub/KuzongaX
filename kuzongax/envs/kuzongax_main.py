...
import json
import os
from kuzongax.grading.grader import Grader
from kuzongax.utils.logger import EpisodeLogger
from kuzongax.utils.util import get_llm_registry, get_utc_date, get_utc_day, get_utc_hour


BASE_DIR='./kuzongax/envs/logs'
RESULTS_DIR = './kuzongax/results'
LEADERBOARDS_DIR = './kuzongax/leaderboards'
# categories
...
if __name__ == "__main__":
    ...
    if data:
        ...
        for key, value in data.items():
            kuzongax = KuzongaX(state=value[ANSWER])
            kuzongax.start()
            # add proximity
            value[PROXIMITY] = kuzongax.get_proximity()
            # add score
            #   because KuzongaX is deterministic, only 100% proximity (meaning exact match to the correct answer) gets a score of 1
